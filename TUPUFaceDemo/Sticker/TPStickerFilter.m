...
#import "TPStickerFilter.h"
#import "TPSticker+OpenGL.h"

@implementation TPStickerFilter
{
...
- (void)setSticker:(TPSticker *)sticker
{
...
{
    if (self.stickerDelegate != nil) {
        runAsynchronouslyOnVideoProcessingQueue(^{
            glFinish();
            CVPixelBufferRef pixelbuffer = firstInputFramebuffer.pixelBuffer;
            [self.stickerDelegate willProcessFrame:firstInputFramebuffer];
        });
    }
...
- (void)renderToTextureWithVertices:(const GLfloat *)vertices textureCoordinates:(const GLfloat *)textureCoordinates;
{
    /*
    for (int i = 0; i < 4; i++) {
        NSLog(@"%f %f", textureCoordinates[2*i], textureCoordinates[2*i+1]);
    }
    NSLog(@"---");
     */
