...
@implementation TPStickersManager
{
...
+ (instancetype)sharedManager
{
    ...
    dispatch_once(&onceToken, ^{
        _stickersManager = [TPStickersManager new];
    });
    
    return _stickersManager;
}

+ (void)loadStickersWithCompletion:(void (^)(NSArray<TPSticker *> *))completion
{
...
- (instancetype)init
{
    ...
    if (self) {
        ...
        NSString *path = [[NSBundle mainBundle] pathForResource:@"TPStickerResources" ofType:@"bundle"];
...
#pragma mark - Private
- (void)_loadStickersWithCompletion:(void(^)(NSArray<TPSticker *> *))completion
{
...
- (NSArray *)_loadStickers
{
    ...
    for (NSURL *fileURL in sortedFiles) {
        TPSticker *sticker = [[TPSticker alloc] initWithDirectoryURL:fileURL];
        if (sticker) {
...
- (instancetype)initWithDirectoryURL:(NSURL *)url
{
    ...
    if (self) {
        ...
        for (NSDictionary *itemDict in itemsDict) {
            TPStickerItem *item = [[TPStickerItem alloc] initWithJSONDictionary:itemDict];
