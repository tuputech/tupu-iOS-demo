...
#import "TPSticker+OpenGL.h"
#import "TPStickerItem+OpenGL.h"


@implementation TPSticker (OpenGL)
                     ...
                     usingBlock:(void (^)(GLfloat *, GLuint))block
{
    ...
    CGPoint eye_left = [points[79] CGPointValue];
    CGPoint eye_right = [points[81] CGPointValue];
    ...
    for (TPStickerItem *item in self.items) {
        switch (item.type) {
            case TPStickerItemTypeFace:
            {
            ...
            case TPStickerItemTypeScreen:
            {
                ...
                switch (item.alignPosition) {
                    case TPStickerItemAlignPositionTop:
                    {
                    ...
                    case TPStickerItemAlignPositionLeft:
                    case TPStickerItemAlignPositionBottom:
                    case TPStickerItemAlignPositionRight:
                    case TPStickerItemAlignPositionCenter:
                    {
                        // TODO: 其它位置的适配
                    }
                        break;
